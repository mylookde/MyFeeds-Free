...
class MyFeeds_Feed_Manager {
    ...
    public function render_feeds_page() {
        ...
        var myFeedsThousandsSep = '<?php global $wp_locale; echo esc_js($wp_locale ? $wp_locale->number_format["thousands_sep"] : ","); ?>';
        function formatNumber(n) {
            n = parseInt(n) || 0;
            if (n === 0) return '0';
            return n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, myFeedsThousandsSep);
        }
        </script>
        <script>
        ...
        .myfeeds-primary-action {
            background: linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%);
            border-color: #c4b5fd;
        }
        
        .myfeeds-secondary-action {
            background: linear-gradient(135deg, #f5f5f5 0%, #efefef 100%);
            border-color: #ddd;
        }
        
        .myfeeds-action-card .description {
            margin-top: 8px;
            margin-bottom: 0;
            font-size: 12px;
            color: #666;
        ...
        /* Auto-Sync Info — positioned next to buttons, pushed right */
        .myfeeds-auto-sync-compact {
            margin-left: auto;
        }
        
        /* HARMONIZED BUTTONS - Same height and style */
        .myfeeds-action-btn {
        ...
        /* Import Panel - Hellblau statt Gelb */
        .myfeeds-import-panel {
        ...
        .myfeeds-import-panel.completed {
            background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%);
            border-color: #28a745;
        }
        
        .myfeeds-progress-bar {
            background: #e9ecef;
            height: 24px;
            border-radius: 12px;
            overflow: hidden;
        ...
        .myfeeds-progress-fill.single-feed {
            background: linear-gradient(90deg, #ff9800, #f57c00);
        }
        
        .myfeeds-progress-fill.completed {
            background: linear-gradient(90deg, #28a745, #20c997);
        }
        
        .myfeeds-import-panel h4 {
            margin: 0 0 10px 0;
            font-size: 16px;
        }
        
        .myfeeds-import-stats {
            color: #555;
            font-size: 13px;
        }
        ...
        .myfeeds-success-message .close-btn {
        ...
        .myfeeds-success-message .close-btn:hover {
            background: rgba(255,255,255,0.3);
        }
        
        .myfeeds-intro-banner {
        ...
        .myfeeds-intro-banner h2 {
            margin: 0 0 15px 0;
            color: white;
        }
        
        .myfeeds-intro-banner p {
            font-size: 16px;
            margin: 0 0 20px 0;
            opacity: 0.9;
        }
    ...
    private function render_feed_modal() {
        ...
        <style>
        .myfeeds-feed-modal-content {
            max-width: 700px;
        }
        .myfeeds-feed-modal-content .myfeeds-help-text {
            background: #f5f3ff;
            border-left: 4px solid #667eea;
            padding: 15px;
            margin-bottom: 20px;
        }
        .myfeeds-feed-modal-content .myfeeds-help-text p {
            margin: 8px 0;
        }
        .myfeeds-feed-modal-content .myfeeds-advanced-options {
            border-top: 1px solid #ddd;
            padding-top: 20px;
        }
        .myfeeds-feed-modal-content .form-table th {
            width: 140px;
            padding: 12px 10px 12px 0;
        }
        .myfeeds-feed-modal-content .form-table td {
            padding: 12px 0;
        }
        .myfeeds-feed-modal-content .large-text {
            width: 100%;
        }
        /* Loading indicator */
        .myfeeds-feed-saving-notice {
            background: #fff;
            border-left: 4px solid #667eea;
            padding: 12px 16px;
            margin: 10px 0;
        ...
        .myfeeds-feed-saving-notice .spinner {
            float: none;
            margin: 0;
            visibility: visible;
        }
        ...
        jQuery(document).ready(function($) {
            ...
            $(document).on('click', '#myfeeds-add-feed-btn', function(e) {
                e.preventDefault();
                $title.text('<?php echo esc_js(__('Add New Feed', 'myfeeds')); ?>');
                $submitBtn.val('<?php echo esc_js(__('Create Feed', 'myfeeds')); ?>');
                $feedKey.val('');
                $('#feed_name').val('');
                $('#feed_url').val('');
            ...
            $(document).on('click', '.myfeeds-edit-feed-btn', function(e) {
                ...
                $title.text('<?php echo esc_js(__('Edit Feed:', 'myfeeds')); ?> ' + name);
                $submitBtn.val('<?php echo esc_js(__('Save Changes', 'myfeeds')); ?>');
                $feedKey.val(key);
                $('#feed_name').val(name);
                $('#feed_url').val(url);
            ...
            $form.on('submit', function(e) {
                ...
                if (!feedName || !feedUrl) {
                    alert('<?php echo esc_js(__('Feed name and URL are required.', 'myfeeds')); ?>');
                    return;
                }
                
                isSaving = true;
                var isEdit = $feedKey.val() !== '';
                var actionLabel = isEdit
                    ? '<?php echo esc_js(__('Saving changes...', 'myfeeds')); ?>'
                    : '<?php echo esc_js(__('Adding feed...', 'myfeeds')); ?>';
                    ...
                    .done(function(response) {
                        if (response.success) {
                            ...
                            if (response.data.import_scheduled && response.data.action === 'created') {
                                ...
                                setTimeout(function() {
                                    window.location.href = '<?php echo esc_url(admin_url('admin.php?page=myfeeds-feeds')); ?>&myfeeds_success=' + encodeURIComponent(response.data.message);
                                }, 800);
                            }
                        } else {
                            // Show error
                            ...
                            $notice.css('border-left-color', '#dc3232')
                                .html('<span style="color:#dc3232;font-weight:600;">&#10007;</span> ' + (response.data ? response.data.message : '<?php echo esc_js(__('An unknown error occurred.', 'myfeeds')); ?>'));
                            setTimeout(function() { $notice.fadeOut(400, function() { $(this).remove(); }); }, 6000);
                        }
                    })
                    .fail(function(xhr) {
                        isSaving = false;
                        var msg = '<?php echo esc_js(__('Server error. Please try again.', 'myfeeds')); ?>';
    ...
    private function render_feeds_table($feeds) {
        ...
        <style>
        .myfeeds-action-buttons form { display: inline-block; margin-right: 5px; }
        .status-active { color: #00a32a; font-weight: bold; }
        .status-inactive { color: #d63638; font-weight: bold; }
        
        /* Feed Status Badges */
        .myfeeds-reimport-btn:disabled {
            opacity: 0.5;
            cursor: not-allowed;
        }
        ...
        .feed-status-importing {
            ...
            animation: pulse-import 1.5s ease-in-out infinite;
        }
        @keyframes pulse-import {
            0%, 100% { opacity: 1; }
            50% { opacity: 0.5; }
        }
        .feed-status-untested {
            background: #fff3e0;
            color: #e65100;
        ...
        /* Inactive feed rows (plan limit exceeded) */
        tr.myfeeds-feed-inactive {
            opacity: 0.45;
        }
        tr.myfeeds-feed-inactive td {
            background: #f8f8f8 !important;
        }
        tr.myfeeds-feed-inactive td:last-child {
            opacity: 1;
        }
        tr.myfeeds-feed-inactive .button-link-delete {
            opacity: 1 !important;
        }
        ...
        .myfeeds-network-badge.awin { background: #e74c3c; }
        .myfeeds-network-badge.tradedoubler { background: #3498db; }
        .myfeeds-network-badge.amazon { background: #ff9900; }
        .myfeeds-network-badge.ebay { background: #0064d3; }
        .myfeeds-network-badge.auto-detected { background: #27ae60; }
        
        .myfeeds-confidence-bar {
            width: 80px;
            height: 16px;
            background: #eee;
            border-radius: 8px;
        ...
        .myfeeds-confidence-fill {
            height: 100%;
            background: linear-gradient(90deg, #e74c3c 0%, #f39c12 50%, #27ae60 100%);
            transition: width 0.3s ease;
        }
        
        .myfeeds-confidence-text {
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
        ...
        .myfeeds-confidence-unknown {
            color: #666;
            font-size: 12px;
            font-style: italic;
        }
        
        .myfeeds-empty-state h3 {
            margin: 0 0 10px 0;
            color: #666;
        }
        
        .myfeeds-empty-state p {
            color: #999;
            margin: 0;
        }
        
        /* Quality Modal */
        .myfeeds-modal-overlay {
            position: fixed;
        ...
        .myfeeds-modal-header h3 { margin: 0; font-size: 16px; }
        .myfeeds-modal-close {
            background: none; border: none; font-size: 24px; cursor: pointer;
            color: #666; padding: 0; line-height: 1;
        }
        .myfeeds-modal-close:hover { color: #333; }
        .myfeeds-modal-body { padding: 20px; }
        .myfeeds-quality-field-row {
            display: flex;
            justify-content: space-between;
            padding: 8px 0;
            border-bottom: 1px solid #f0f0f0;
        }
        .myfeeds-quality-field-name { font-weight: 500; }
        .myfeeds-quality-field-missing { color: #d63638; }
        .myfeeds-quality-field-ok { color: #00a32a; }
        .myfeeds-quality-field-label {
            font-size: 11px;
            color: #888;
            text-transform: uppercase;
        }
        .myfeeds-quality-product-item {
            background: #f8f9fa;
        ...
        .myfeeds-quality-product-item strong { display: block; margin-bottom: 4px; }
        .myfeeds-quality-product-item code { font-size: 11px; color: #666; }
        .myfeeds-quality-product-item .missing-tags {
            margin-top: 6px;
        }
        .myfeeds-quality-product-item .missing-tags span {
            display: inline-block;
            background: #ffeaea;
            color: #d63638;
            padding: 2px 6px;
        ...
        .myfeeds-quality-clickable:hover {
            opacity: 0.8;
            box-shadow: 0 0 0 2px #2271b1;
            border-radius: 8px;
        }
        
        /* Feeds table header with Add button */
        .myfeeds-feeds-table-header {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-bottom: 10px;
        }
        .myfeeds-feeds-table-header h2 {
            margin: 0;
        }
        .myfeeds-feeds-table {
            background: #fff;
