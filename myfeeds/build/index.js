(() => {
  ...
  const { TextControl, Button, Modal, Spinner, PanelBody, ToggleControl, RangeControl, SelectControl } = window.wp.components;
  ...
  // Count total selected products across ALL myfeeds/product-picker blocks on the current page
  // If excludeBlockId is provided, skip that block (for accurate counting when own state hasn't synced yet)
  function getTotalSelectedProductsOnPage(excludeBlockId) {
    if (!window.wp || !window.wp.data || !window.wp.data.select) return 0;
    var store = window.wp.data.select('core/block-editor');
    if (!store || !store.getBlocks) return 0;
    var allBlocks = store.getBlocks();
    var total = 0;
    function countInBlocks(blocks) {
      blocks.forEach(function(block) {
        if (block.name === 'myfeeds/product-picker') {
          if (excludeBlockId && block.clientId === excludeBlockId) return;
          var products = (block.attributes && block.attributes.selectedProducts) || [];
          total += products.length;
        }
        if (block.innerBlocks && block.innerBlocks.length > 0) {
          countInBlocks(block.innerBlocks);
        }
      });
    }
    countInBlocks(allBlocks);
    return total;
  }
  ...
  registerBlockType("myfeeds/product-picker", {
    ...
    attributes: {
      ...
      autoplayInterval: { type: "number", default: 3 },
      arrowColor: { type: "string", default: "#333333" },
    },
    ...
    edit({ attributes, setAttributes, clientId }) {
      ...
      const [hexInput, setHexInput] = useState((attributes.arrowColor || "#333333").replace("#", ""));
      ...
      // Fetch plan limits on mount (not just when modal opens)
      useEffect(function() {
        if (apiUrl) {
          fetch(apiUrl + 'plan-limits', {
            credentials: 'include',
            headers: { 'X-WP-Nonce': nonce }
          })
          .then(function(r) { return r.json(); })
          .then(function(data) { if (data && data.plan) setPlanLimits(data); })
          .catch(function() { /* Silently fail — behave as Pro */ });
        }
      }, []);

      // Sync hex input ONLY when arrowColor changes from palette click (not from typing)
      const [hexFromPalette, setHexFromPalette] = useState(false);
      useEffect(function() {
        if (hexFromPalette) {
          setHexInput((attributes.arrowColor || "#333333").replace("#", ""));
          setHexFromPalette(false);
        }
      }, [attributes.arrowColor]);

      // Update total selected products across all blocks — subscribe to editor store
      // so changes in OTHER blocks (clear all, duplicate, delete) are detected too
      useEffect(function() {
        // Compute: other blocks from store + own selected.length (avoids timing issues)
        function recompute() {
          var othersCount = getTotalSelectedProductsOnPage(clientId);
          return othersCount + selected.length;
        }
        setTotalOnPage(recompute());
        
        // Subscribe to block editor changes (catches edits in ALL blocks)
        var unsubscribe = wp.data.subscribe(function() {
          var newTotal = recompute();
          setTotalOnPage(function(prev) {
      ...
      const toggleProduct = function (product, useCurrentVariant) {
        ...
        // Check if this is an ADD (not a remove) and if limit is reached
        const isAlreadySelected = (selected || []).some(function(p){ return String(p.id) === String(awId); });
        if (!isAlreadySelected) {
            // Check limit: totalOnPage counts products across ALL blocks on this page
            var maxProducts = (planLimits && planLimits.max_products !== undefined && planLimits.max_products < 999999) ? planLimits.max_products : Infinity;
            if (totalOnPage >= maxProducts) {
                // Don't add — limit reached. The UI already shows greyed-out cards.
                return;
            }
        }
      ...
      const productCard = function(product, index){
        ...
        var maxProducts = (planLimits && planLimits.max_products !== undefined && planLimits.max_products < 999999) ? planLimits.max_products : Infinity;
        var limitReached = totalOnPage >= maxProducts;
        var isDisabledByLimit = limitReached && !isSelected;
        ...
        return React.createElement("div", { key: 'result-' + String(product.id || product.aw_product_id) + '-' + index, style: Object.assign({}, { border: isSelected ? "2px solid #667eea" : "1px solid #e5e7eb", borderRadius: "8px", background: "#fff", cursor: isDisabledByLimit ? "default" : "pointer", overflow: "hidden", transition: "box-shadow .2s", boxShadow: isSelected ? "0 0 0 2px rgba(102,126,234,0.15)" : "0 1px 2px rgba(0,0,0,0.06)" }, isDisabledByLimit ? { opacity: 0.4, pointerEvents: 'none' } : {}), onClick: function(){ toggleProduct(product); } },
      ...
      return React.createElement(
        ...
        // Inspector Controls (Block Sidebar)
        React.createElement(InspectorControls, null,
          React.createElement(PanelBody, { title: "Display Settings", initialOpen: true },
            React.createElement(SelectControl, {
              ...
              onChange: function(val) {
                if (val === 'carousel' && planLimits && planLimits.plan === 'Free') {
                  return;
                }
                setAttributes({ displayMode: val });
              }
            }),
            planLimits && planLimits.plan === 'Free' && attributes.displayMode !== 'carousel' ?
              React.createElement("p", { style: { fontSize: "12px", color: "#667eea" } },
                "🔒 Carousel mode is a Pro feature. ",
                React.createElement("a", { href: planLimits.upgrade_url || '#', target: "_blank" }, "Upgrade →")
              ) : null,
            attributes.displayMode === 'carousel' ? React.createElement(ToggleControl, {
              label: "Auto-play",
              checked: attributes.autoplay,
              onChange: function(val) { setAttributes({ autoplay: val }); }
            }) : null,
            attributes.displayMode === 'carousel' && attributes.autoplay ? React.createElement(RangeControl, {
              label: "Slide interval (seconds)",
              value: attributes.autoplayInterval,
              onChange: function(val) { setAttributes({ autoplayInterval: val }); },
              min: 2,
              max: 15,
              step: 1
            }) : null,
            attributes.displayMode === 'carousel' ? React.createElement("div", { style: { marginTop: "16px" } },
              React.createElement("label", { style: { display: "block", marginBottom: "8px", fontSize: "11px", fontWeight: "500", textTransform: "uppercase" } }, "Arrow Color"),
              // Quick-pick palette
              React.createElement("div", { style: { display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px" } },
                [
                  ...
                  { name: "Warm Brown", color: "#a0826d" },
                  { name: "Beige", color: "#c8b89a" },
                  { name: "Pink", color: "#e91e63" },
                  { name: "Green", color: "#27ae60" },
                  { name: "Orange", color: "#f39c12" },
                ].map(function(c) {
                  var isActive = (attributes.arrowColor || "#333333") === c.color;
                  return React.createElement("div", {
                    key: c.color,
                    title: c.name,
                    onClick: function() { setHexFromPalette(true); setAttributes({ arrowColor: c.color }); },
                    style: {
                      width: "28px",
                      height: "28px",
                      borderRadius: "50%",
                      ...
                      cursor: "pointer",
                      border: isActive ? "3px solid #667eea" : "2px solid #ddd",
                      boxShadow: isActive ? "0 0 0 2px #fff, 0 0 0 4px #667eea" : "none",
              ...
              React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
                React.createElement("span", { style: { fontSize: "13px", color: "#666" } }, "#"),
                React.createElement("input", {
                  type: "text",
                  value: hexInput,
                  onChange: function(e) {
                    var raw = e.target.value.replace(/[^0-9a-fA-F]/g, "").substring(0, 6);
                    setHexInput(raw);
                  },
                  onBlur: function() {
                    var raw = hexInput.replace(/[^0-9a-fA-F]/g, "").substring(0, 6);
                    if (raw.length === 6) {
                      setAttributes({ arrowColor: "#" + raw });
                    } else if (raw.length === 3) {
                      var expanded = raw[0] + raw[0] + raw[1] + raw[1] + raw[2] + raw[2];
                      setAttributes({ arrowColor: "#" + expanded });
                    } else {
                      setHexInput((attributes.arrowColor || "#333333").replace("#", ""));
                    }
                  },
                ...
                React.createElement("div", {
                  style: {
                    ...
                    height: "24px",
                    borderRadius: "50%",
                    backgroundColor: attributes.arrowColor || "#333333",
                    border: "1px solid #ddd",
                    flexShrink: 0
                  }
                })
        ...
        // Carousel mode banner (editor preview hint)
        attributes.displayMode === 'carousel' && selected.length > 0 && React.createElement("div", {
          style: {
        ...
        }, data.isFree ? "Carousel is a Pro feature — Upgrade to enable carousel mode" : "Carousel mode active — Preview only visible on frontend"),
        ...
        showModal && !showProductDetail && React.createElement(Modal, { title: results.length > 0 ? ((!searchTerm || searchTerm.length < 2) ? "Your Selected Products (" + selected.length + ")" : "Product Search Results (" + results.length + " found)") : "Product Search Results", onRequestClose: function(){ setShowModal(false); }, className: "myfeeds-spacious-modal", shouldCloseOnClickOutside: false },
          React.createElement("div", { className: "myfeeds-modal-content" },
            ...
            // Plan limit banner / Upgrade prompt — rendered AFTER sticky search controls so it scrolls normally
            (function(){
              if (!planLimits || planLimits.plan === 'Pro') return null;
              var usedCount = totalOnPage;
              var maxCount = (planLimits.max_products !== undefined && planLimits.max_products < 999999) ? planLimits.max_products : null;
              if (maxCount === null) return null;
              var canSelect = totalOnPage < maxCount;
              // Under limit: show subtle info banner
              if (canSelect) {
                return React.createElement("div", { style: { backgroundColor: "#f5f3ff", border: "1px solid #c4b5fd", padding: "10px 14px", borderRadius: "6px", margin: "40px 0 20px 0", fontSize: "13px", color: "#4c1d95" }, "data-testid": "plan-limit-banner" },
                  "Free plan: " + usedCount + " of " + maxCount + " products used. ",
                  React.createElement("a", { href: planLimits.upgrade_url || '#', target: "_blank", rel: "noopener", style: { color: "#667eea", fontWeight: 600 } }, "Upgrade to Pro \u2192")
                );
              }
              // Limit reached: show prominent centered upgrade block
              return React.createElement("div", { style: { textAlign: "center", padding: "30px 24px", margin: "40px 0 20px 0", background: "linear-gradient(135deg, #f5f3ff 0%, #ede9fe 50%, #e8f4fd 100%)", borderRadius: "12px", border: "1px solid #c4b5fd" }, "data-testid": "plan-limit-banner" },
                React.createElement("div", { style: { fontSize: "32px", marginBottom: "8px" } }, "\uD83D\uDD12"),
                React.createElement("h3", { style: { margin: "0 0 8px 0", fontSize: "18px", fontWeight: "700", color: "#4c1d95" } }, "Product Limit Reached"),
                React.createElement("p", { style: { margin: "0 0 16px 0", fontSize: "14px", color: "#6b7280", maxWidth: "400px", marginLeft: "auto", marginRight: "auto" } },
                  "You\u2019ve selected " + usedCount + " of " + maxCount + " products on this page. Upgrade to Pro for unlimited products, unlimited feeds, and automatic daily sync."
                ),
                React.createElement("a", { href: planLimits.upgrade_url || '#', target: "_blank", rel: "noopener", style: { display: "inline-block", padding: "10px 24px", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "#fff", borderRadius: "6px", textDecoration: "none", fontWeight: "600", fontSize: "14px", boxShadow: "0 2px 8px rgba(102, 126, 234, 0.3)" } }, "\uD83D\uDE80 Upgrade to Pro")
              );
            })(),
        ...
        showProductDetail && React.createElement(Modal, { title: "", onRequestClose: function(){ 
          ...
          React.createElement("div", { className: "myfeeds-product-actions" },
            ...
            React.createElement("button", { 
              ...
              disabled: (function(){
                // Disable if limit reached and product is not already selected (only block Add, not Remove)
                var isAlready = (selected || []).some(function(p){ return String(p.id) === String(showProductDetail && showProductDetail.id); });
                if (!isAlready) {
                    var maxProducts = (planLimits && planLimits.max_products !== undefined && planLimits.max_products < 999999) ? planLimits.max_products : Infinity;
                    if (totalOnPage >= maxProducts) return true;
                }
                return false;
              })(),
              style: (function(){
                var isAlready = (selected || []).some(function(p){ return String(p.id) === String(showProductDetail && showProductDetail.id); });
                var maxProducts = (planLimits && planLimits.max_products !== undefined && planLimits.max_products < 999999) ? planLimits.max_products : Infinity;
                var isDisabled = !isAlready && totalOnPage >= maxProducts;
                return isDisabled ? { opacity: 0.5, cursor: 'not-allowed' } : {};
              })(),
              onClick: function(){ 
                toggleProduct(showProductDetail, true); // CRITICAL: Pass true to use currentVariant
                setShowProductDetail(null); 
              }
